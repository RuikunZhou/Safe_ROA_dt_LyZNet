...
def plot_V(system, net=None, model_path=None, V_list=None, c_lists=None, 
           ...
           obstacles=None, plot_control=True, h=None, bias_correction=False):
    ...
    # Compute bias correction if enabled
    V_bias = 0
    DV_bias = 0
    if bias_correction and net is not None:
        x_zero = torch.zeros((1, len(system.symbolic_vars)), dtype=torch.float32).to(device)
        x_zero.requires_grad = True
        V_zero = net(x_zero).squeeze()
        DV_bias = torch.autograd.grad(V_zero.sum(), x_zero, create_graph=False)[0].detach().cpu().numpy()
        V_bias = V_zero.item()
    ...
    if d == 1:
        ...
        if elm_model is not None: 
            ...
            if bias_correction:
                V_test = V_test - (input_array @ DV_bias.T + V_bias)
        ...
        if elm_model is not None: 
            ...
            if bias_correction:
                V_test = V_test - (input_array @ DV_bias.T + V_bias).reshape(x1.shape)
        ...
        if c1_V is not None:
            cs1 = ax2.contour(x1, x2, V_test, colors='b', levels=[c1_V])
            ax2.clabel(cs1, inline=1, fontsize=6)
        
        if c2_V is not None:
            cs2 = ax2.contour(x1, x2, V_test, colors='b', levels=[c2_V], 
                              linewidths=3)    
    ...
    if h is not None:
        h_values = np.array([h(xi) for xi in input_array]).reshape(x1.shape)
        ax2.contourf(x1, x2, h_values, levels=[1, np.inf], colors=['grey'], 
                     alpha=0.7)
