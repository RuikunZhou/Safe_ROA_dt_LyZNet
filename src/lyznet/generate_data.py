...
def generate_data(system, n_samples=None, v_max=200, 
                  ...
                  transform="tanh", omega=None, T=1, eps=1e-7,
                  X_MAX=1e+6):
    ...
    if plot:  
        ...
        if z_values is not None: 
            # print the largest z values
            print("Largest z values: ", np.max(z_values))
            print("Mean of z values: ", np.mean(z_values))
...
def generate_dts_data(system, n_samples=None, v_max=200, 
                      overwrite=False, n_nodes=32, plot=True, h=None):
    domain = system.domain
    d = len(domain)  
    ...
    mu = 40/v_max  # 1 - exp(-40) is practically 1

    # def augmented_dynamics_discrete(x, z):
    #     dz = sum([s**2 for s in x])  # stage cost

    def augmented_dynamics_discrete(x, z):
        x = np.array(x, dtype=np.float64).flatten()  # Ensure consistent type
        h_value = h(x) if h is not None else None

        if h is not None and h_value >= 1:
            z_next = v_max + 1
        else:
            gamma = 1 + 1 / (1 - h_value) if h is not None else 1
            dz = sum([s**2 for s in x]) * gamma
            z_next = z + dz
