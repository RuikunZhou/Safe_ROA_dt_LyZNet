...
def local_stability_verifier(system, tol=1e-4, accuracy=1e-5, h=None):
    ...
    if system.system_type == "DiscreteDynamicalSystem":
        # Discrete-time Lyapunov condition: V(Ax) - V(x)
        Q = (system.A.T @ system.P @ system.A - system.P).astype(np.float64)
        r = np.max(np.abs(np.linalg.eigvals(Q))) - epsilon

        if r < 0:
            print("V(f) - V(x) is not negative definite: the system is not locally stable!")
            return None
    else:
        # Continuous-time Lyapunov condition: DV*f
        Q = (-system.P @ system.A - system.A.T @ system.P).astype(np.float64)
        r = np.min(np.linalg.eigvalsh(Q)) - epsilon

        if r < 0:
            print("V is not a Lyapunov function: DV*f is not negative definite!")
            return None
    ...
    if system.system_type == "DiscreteDynamicalSystem": 
        ...
        norm_sq_bound = 2*dreal.sqrt(frobenius_norm_sq_A_P_Dg) + P_norm*frobenius_norm_sq_Dg
        hessian_condition = norm_sq_bound <= r
    else:
        # Frobenios norm as an overa-approximation of 2-norm
        # frobenius_norm_sq_P_Dg = sum(sum(m_ij**2 for m_ij in row) for row in P_Dg)
        # hessian_condition = (2**2 * frobenius_norm_sq_P_Dg) <= r**2
        P_Dg_symmetric = P_Dg + P_Dg.T
        # print("P_Dg_symmetric: ", P_Dg_symmetric)
        frobenius_norm_sq_P_Dg = sum(sum(m_ij**2 for m_ij in row) for row in P_Dg_symmetric)
        hessian_condition = frobenius_norm_sq_P_Dg <= r**2

    start_time = time.time()
    if dreal.CheckSatisfiability(dreal.logical_not(hessian_condition), config) is None:
        end_time = time.time()
    ...
    def Check_local_stability(c):    
        ...
        stability = dreal.logical_imply(
            dreal.logical_and(omega, x_bound), hessian_condition
            )
        ...
        #     condition = dreal.logical_and(condition, safety)

        return dreal.CheckSatisfiability(dreal.logical_not(stability), config)
...
def local_stabilization_verifier(system, tol=1e-4, accuracy=1e-5, c_max=100):
    ...
    f_u = lyznet.get_closed_loop_f_expr(system.symbolic_f, system.symbolic_g, 
                                        u, system.symbolic_vars)
    closed_loop_sys = lyznet.DynamicalSystem(f_u, system.domain, system.name)
    # use P from LQR solution instead of solving P again
