...
def find_ep(system, method="symbolic", num_attempts=10, u_value=None):
    """
    Find equilibrium points of a dynamical or control affine system.

    Parameters:
    - system: DynamicalSystem or ControlAffineSystem
    - method: str, "symbolic" or "numerical"
    - num_attempts: int, number of random initial guesses for numerical method
    - u_value: Control input value(s) to be used during equilibrium computation.
               If None, defaults to zero control input.

    Returns:
    - list of equilibrium points as lists of floats (x_e for control systems)
    """
    is_control_system = system.system_type == "ControlAffineSystem"
    ...
    if is_control_system:
        ...
        if u_value is None:
            u_value = [0] * g.shape[1]  # Default to zero control input
        elif isinstance(u_value, (int, float)):
            u_value = [u_value] * g.shape[1]  # If scalar, replicate for all controls
        elif len(u_value) != g.shape[1]:
            raise ValueError("Length of u_value must match number of control inputs.")

        # Substitute control inputs into the dynamics
        control_vars = [sympy.symbols(f'u{i+1}') for i in range(g.shape[1])]
        control_subs = dict(zip(control_vars, u_value))
        full_dynamics = f + g @ sympy.Matrix(control_vars)
        full_dynamics = full_dynamics.subs(control_subs)
    else:
        f = system.symbolic_f
        symbolic_vars = system.symbolic_vars
        full_dynamics = f
    ...
    if method == "symbolic":
        try:
            solutions = sympy.solve(full_dynamics, symbolic_vars, dict=True)
            if not solutions:
                raise ValueError("No symbolic solutions found. Switching to numerical solving.")
            # Extract solutions within the domain
            equilibrium_points = []
            for sol in solutions:
                point = [float(sol[var]) for var in symbolic_vars]
                if all(d[0] <= val <= d[1] for val, d in zip(point, domain)):
                    equilibrium_points.append(point)
            print("Equilibrium points found: ", equilibrium_points)                    
            return equilibrium_points
        except (NotImplementedError, ValueError) as e:
            print(f"Symbolic solving failed: {e}. Proceeding with numerical solving.")
            method = "numerical"

    if method == "numerical":
        if domain is None:
            raise ValueError("Domain must be specified for numerical method.")

        # Convert sympy expressions to numerical functions
        full_dynamics_numeric = sympy.lambdify(symbolic_vars, full_dynamics, modules='numpy')

        def wrapped_f_numeric(variables):
            return numpy.squeeze(full_dynamics_numeric(*variables))  # Unpack array into individual arguments

        equilibrium_points = []

        for _ in range(num_attempts):
            # Generate random initial guess within the domain
            initial_guess = numpy.array([numpy.random.uniform(d[0], d[1]) for d in domain])

            # Solve using a numerical root finder
            sol = scipy.optimize.root(wrapped_f_numeric, initial_guess)

            if sol.success:
                eq_point = numpy.round(sol.x, decimals=6).tolist()
                # Check if the solution is within the domain and unique
                if all(d[0] <= val <= d[1] for val, d in zip(eq_point, domain)):
                    if eq_point not in equilibrium_points:
...
# def find_ep(system, method="symbolic", num_attempts=10):
#     """
#     Find equilibrium points of a dynamical system using its domain.
    
#     Parameters:
#     - system: DynamicalSystem, the system for which to find equilibrium points
#     - method: str, "symbolic" or "numerical"
#     - num_attempts: int, number of random initial guesses for numerical method
    
#     Returns:
#     - list of equilibrium points as lists of floats
#     """
#     f = system.symbolic_f
#     symbolic_vars = system.symbolic_vars
#     domain = system.domain
...
#     if method == "symbolic":
#         try:
#             solutions = sympy.solve(f, symbolic_vars, dict=True)
#             if not solutions:
#                 raise ValueError("No symbolic solutions found. Switching to numerical solving.")
#             return [[float(sol[var]) for var in symbolic_vars] for sol in solutions]
#         except (NotImplementedError, ValueError) as e:
#             print(f"Symbolic solving failed: {e}. Proceeding with numerical solving.")
#             method = "numerical"
...
#         # Convert sympy expressions to numerical functions
#         f_numeric = sympy.lambdify(symbolic_vars, f, modules='numpy')
        
#         def wrapped_f_numeric(x):
#             return numpy.squeeze(f_numeric(*x))  # Unpack array into individual arguments
...
def sympy_to_z3(expr, subs, func_subs=None, extra_constraints=None):
    ...
    if expr.is_Add:
        ...
        non_zero_terms = [
            sympy_to_z3(arg, subs, func_subs, extra_constraints)
            for arg in expr.args if not arg.is_zero
        ]
        if len(non_zero_terms) == 0:
            return z3.RealVal(0)
        else:
            result = non_zero_terms[0]
            for term in non_zero_terms[1:]:
                result = result + term
            return result

    elif expr.is_Mul:
        # Detect and handle cases where -1 is explicitly a multiplier
        if expr.args[0] == -1 and len(expr.args) == 2:
            # -1 * expr => -sympy_to_z3(expr)
            return -sympy_to_z3(expr.args[1], subs, func_subs, extra_constraints)
        else:
            non_zero_factors = [
                sympy_to_z3(arg, subs, func_subs, extra_constraints)
                for arg in expr.args if not arg.is_zero
            ...
            if len(non_zero_factors) == 0:
                return z3.RealVal(1)  # Multiplicative identity
            else:
                result = non_zero_factors[0]
                for factor in non_zero_factors[1:]:
        ...
        if expr.exp.is_Integer and expr.exp >= 0:
            ...
            # Special handling for -1 exponent
            return z3.RealVal(1) / base
        elif expr.exp.is_Integer and expr.exp < 0:
            # General handling for negative integer exponents
            return z3.RealVal(1) / (base ** abs(int(expr.exp)))
        else:
        ...
        # Handle absolute value function
            ...
            arg_z3 = sympy_to_z3(expr.args[0], subs, func_subs, extra_constraints)
            return z3.If(arg_z3 >= 0, arg_z3, -arg_z3)
...
def extract_lowest_homogeneous_part_dreal(f, x):
    k = 0
    while True:
        # Extract homogeneous terms of degree k for each component of f
        f_h = []
        all_zero = True
        for fi in f:
            h_k, is_zero = extract_homo_polynomial_dreal(fi, x, k)
            f_h.append(h_k)
            if not is_zero:
                all_zero = False

        # print(f"Checking degree {k} homogeneous part of f: {f_h}")

        if not all_zero:
            print(f"Lowest-degree non-zero homogeneous part found at degree {k}: {f_h}")
            return f_h, k

        k += 1


def extract_homo_polynomial_dreal(f_dreal, vars_dreal, k, tol=1e-14, 
                                  symbolic_evaluation=False):
    origin = {v: 0 for v in vars_dreal}
    multiindices = product(range(k + 1), repeat=len(vars_dreal))
    multiindices = [alpha for alpha in multiindices if sum(alpha) == k]

    homogeneous_polynomial = dreal.Expression(0)
    is_zero = True

    for alpha in multiindices:
        derivative = f_dreal
        for i, v in enumerate(vars_dreal):
        ...
        if symbolic_evaluation:
            ...
            derivative_at_origin_value = derivative_at_origin.Evaluate()   
        else:
            derivative_at_origin = derivative.Substitute(origin).Evaluate()
            derivative_at_origin_value = derivative_at_origin  
        coefficient = derivative_at_origin / dreal.Expression(prod(factorial(a) for a in alpha if a > 0))

        if abs(derivative_at_origin_value) >= tol:
            is_zero = False

        monomial = dreal.Expression(1)
        for i, v in enumerate(vars_dreal):
            monomial *= v**alpha[i]

        homogeneous_polynomial += coefficient * monomial

    return homogeneous_polynomial, is_zero
...
def dreal_to_sympy(dreal_expr, sympy_variables):
    ...
    # Replace dReal's Variable representation with the corresponding SymPy symbols
    for dreal_name, sympy_var in var_map.items():
        dreal_expr_str = dreal_expr_str.replace(f"Variable('{dreal_name}')", f"var_map['{dreal_name}']")
        dreal_expr_str = dreal_expr_str.replace(dreal_name, f"var_map['{dreal_name}']")
    
    # Replace "pow" with "sympy.Pow" for compatibility with SymPy
    dreal_expr_str = dreal_expr_str.replace("pow", "sympy.Pow")
    ...
    eval_context = {
        ...
        "acos": sympy.acos,
        "abs": sympy.Abs,
        "Pow": sympy.Pow  # Explicit mapping for SymPy's power function
    }
    ...
    try:
        ...
        print(f"Error converting dreal expression: {dreal_expr_str}")
        raise e
...
def simplify_dreal_expression(expr, dreal_vars):
    ...
    simplified_expr_sympy = sympy.simplify(sympy.expand((expr_sympy)))
...
def compute_jacobian_np_dreal(symbolic_f, symbolic_vars, point=None):
    if point is None:
        point = [0] * len(symbolic_vars)
    dreal_vars = [dreal.Variable(str(v)) for v in symbolic_vars]
